...
inline Pixel ScaleColor(Pixel a_Color1, unsigned int a_Scale)
{
  const unsigned int r = (((a_Color1 & REDMASK) * a_Scale) >> 8) & REDMASK;
  const unsigned int g = (((a_Color1 & GREENMASK) * a_Scale) >> 8) & GREENMASK;
  const unsigned int b = (((a_Color1 & BLUEMASK) * a_Scale) >> 8) & BLUEMASK;
